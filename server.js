...
// Automatically deletes all temp-clone-* folders older than 24 hours
function cleanTempClones() {
  console.log('[Server] Running automated temp-clone cleanup check...');
  const baseDir = process.cwd();
  try {
    const files = fs.readdirSync(baseDir);
    const now = Date.now();
    const ageLimit = 24 * 60 * 60 * 1000; // 24 hours in ms
      ...
      if (file.startsWith(TEMP_DIR_PREFIX)) {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            const suffix = file.substring(TEMP_DIR_PREFIX.length);
            const folderTime = parseInt(suffix, 10);
            const finalTime = !isNaN(folderTime) ? folderTime : stats.mtimeMs;

            if (now - finalTime > ageLimit) {
              console.log(`[Server] Deleting expired temp folder: ${file}`);
              fs.rmSync(fullPath, { recursive: true, force: true });
              
              // Also clean from activeTempRepos cache
              for (const [repoUrl, cachedPath] of activeTempRepos.entries()) {
                if (cachedPath === fullPath) {
                  activeTempRepos.delete(repoUrl);
                }
              }
            }
          }
        } catch (fileErr) {
          console.error(`[Server] Error processing temp folder "${file}": ${fileErr.message}`);
        }
      }
    }
  } catch (err) {
    console.error(`[Server] Failed to clean temp clones: ${err.message}`);
  }
}
...
function scheduleMidnightCleanup() {
  ...
  setTimeout(() => {
    cleanTempClones();
    // After the first midnight execution, run every 24 hours
    setInterval(cleanTempClones, 24 * 60 * 60 * 1000);
  }, msUntilMidnight);
}
